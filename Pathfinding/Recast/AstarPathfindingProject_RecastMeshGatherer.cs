	...
	private RasterizationMesh RasterizeCapsuleCollider(float radius, float height, Bounds bounds, Matrix4x4 localToWorldMatrix)
	{
		...
		if (capsuleCache == null)
		{
			...
			for (int j = 0; j < num; j++)
			{
				...
				{
					array[k + j * num2] = new Vector3(Mathf.Cos((float)k * MathF.PI * 2f / (float)num2) * Mathf.Sin((float)j * MathF.PI / (float)(num - 1)), Mathf.Cos((float)j * MathF.PI / (float)(num - 1)) + ((j < num / 2) ? height : (0f - height)), Mathf.Sin((float)k * MathF.PI * 2f / (float)num2) * Mathf.Sin((float)j * MathF.PI / (float)(num - 1)));
				}
			}
			array[^2] = Vector3.down;
			int num3 = 0;
			int item = num2 - 1;
			while (num3 < num2)
			{
