...
{
	public struct Iterator(DynamicArray<T> setOwner)
	{
		private readonly DynamicArray<T> owner = setOwner;
	...
	{
		public struct RangeIterator(DynamicArray<T> setOwner, int first, int numItems)
		{
			private readonly DynamicArray<T> owner = setOwner;
			...
			private int last = first + numItems;
