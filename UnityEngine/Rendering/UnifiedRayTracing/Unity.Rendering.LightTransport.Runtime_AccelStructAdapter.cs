...
internal sealed class AccelStructAdapter : IDisposable
{
	...
	private void AddTrees(TerrainDesc terrainDesc, ref List<InstanceIDs> instanceHandles)
	{
		...
		float4x4 float4x5 = terrainDesc.localToWorldMatrix;
		float3 float5 = new float3(terrainData.heightmapResolution, 1f, terrainData.heightmapResolution) * terrainData.heightmapScale;
		float3 float6 = new float3(float4x5[3].x, float4x5[3].y, float4x5[3].z);
		TreeInstance[] treeInstances = terrainData.treeInstances;
		for (int i = 0; i < treeInstances.Length; i++)
		{
			TreeInstance treeInstance = treeInstances[i];
			Matrix4x4 localToWorldMatrix = Matrix4x4.TRS(float6 + new float3(treeInstance.position) * float5, Quaternion.AngleAxis(treeInstance.rotation, Vector3.up), new Vector3(treeInstance.widthScale, treeInstance.heightScale, treeInstance.widthScale));
