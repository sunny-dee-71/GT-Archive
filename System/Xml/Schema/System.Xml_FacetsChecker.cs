...
{
	private struct FacetsCompiler(DatatypeImplementation baseDatatype, RestrictionFacets restriction)
	{
		private struct Map
		{
			internal char match;
		...
		private DatatypeImplementation datatype = baseDatatype;

		private RestrictionFacets derivedRestriction = restriction;

		private RestrictionFlags baseFlags = ((datatype.Restriction != null) ? datatype.Restriction.Flags : ((RestrictionFlags)0));

		private RestrictionFlags baseFixedFlags = ((datatype.Restriction != null) ? datatype.Restriction.FixedFlags : ((RestrictionFlags)0));

		private RestrictionFlags validRestrictionFlags = datatype.ValidRestrictionFlags;

		private XmlSchemaDatatype nonNegativeInt = DatatypeImplementation.GetSimpleTypeFromTypeCode(XmlTypeCode.NonNegativeInteger).Datatype;

		private XmlSchemaDatatype builtInType = ((builtInEnum > XmlTypeCode.None) ? DatatypeImplementation.GetSimpleTypeFromTypeCode(builtInEnum).Datatype : datatype);

		private XmlTypeCode builtInEnum = ((!(datatype is Datatype_union) && !(datatype is Datatype_List)) ? datatype.TypeCode : XmlTypeCode.None);

		private bool firstPattern = true;

		private StringBuilder regStr = null;

		private XmlSchemaPatternFacet pattern_facet = null;
