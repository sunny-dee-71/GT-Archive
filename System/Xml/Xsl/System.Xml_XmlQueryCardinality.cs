...
internal struct XmlQueryCardinality
{
	...
	{
		this = new XmlQueryCardinality(reader.ReadByte());
	}

	static XmlQueryCardinality()
	{
		cardinalityProduct = new XmlQueryCardinality[8, 8]
		{
		...
		cardinalitySum = new XmlQueryCardinality[8, 8]
		{
			{ None, Zero, One, ZeroOrOne, More, NotOne, OneOrMore, ZeroOrMore },
		...
		toString = new string[8] { "", "?", "", "?", "+", "*", "+", "*" };
		serialized = new string[8] { "None", "Zero", "One", "ZeroOrOne", "More", "NotOne", "OneOrMore", "ZeroOrMore" };
	}
}
