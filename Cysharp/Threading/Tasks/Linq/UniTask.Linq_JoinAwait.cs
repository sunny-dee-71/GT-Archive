...
internal sealed class JoinAwait<TOuter, TInner, TKey, TResult> : IUniTaskAsyncEnumerable<TResult>
{
	private sealed class _JoinAwait : MoveNextSource, IUniTaskAsyncEnumerator<TResult>, IUniTaskAsyncDisposable
	{
		...
		{
			_JoinAwait joinAwait = (_JoinAwait)state;
			if (joinAwait.TryGetResult(joinAwait.awaiter, out var result))
			{
				if (result)
				{
					...
					{
						OuterSelectCore(joinAwait);
						return;
					}
					joinAwait.continueNext = false;
					joinAwait.outerKeyAwaiter.SourceOnCompleted(OuterSelectCoreDelegate, joinAwait);
				}
				else
				{
					joinAwait.continueNext = false;
					joinAwait.completionSource.TrySetResult(result: false);
				}
			}
			else
			{
		...
		{
			_JoinAwait joinAwait = (_JoinAwait)state;
			if (joinAwait.TryGetResult(joinAwait.outerKeyAwaiter, out var result))
			{
				joinAwait.valueEnumerator = joinAwait.lookup[result].GetEnumerator();
		...
		{
			_JoinAwait joinAwait = (_JoinAwait)state;
			if (joinAwait.TryGetResult(joinAwait.resultAwaiter, out var result))
			{
				joinAwait.Current = result;
				joinAwait.completionSource.TrySetResult(result: true);
			}
		}
